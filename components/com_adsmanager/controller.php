...
class AdsManagerController extends TController
{
	...
	function updatedate() {
		...
		$app->redirect( TRoute::_('index.php?option=com_comprofiler&tab=AdsManagerTab'), JText::_('ADSMANAGER_DATE_UPDATED') );
	}
	...
	*/
	...
	{	
		...
		switch($mode) { 
			...
			case "payperad":
				...
				if ($total == 0) {	
					...
					$invoice_id = generateBill($content,$total,$items,$mode,"pending",$adid);
				}
				break;
		...
		if (($mode == "payperad" )&&($total > 0)&&($isUpdateMode ==0)) {
			...
			if (ADSMANAGER_SPECIAL == "abrivac")
				$app->redirect( 'index.php?option=com_invoicing&view=invoice&layout=payment&id='.$invoice_id, "" );
			else
				$app->redirect( 'index.php?option=com_paidsystem&view=payment', "" );
		} else {	
			$cache = & JFactory::getCache('com_adsmanager');
			...
			if ($conf->submission_type == 2)
				...
				$app->redirect( TRoute::_('index.php?option=com_comprofiler&tab=AdsManagerTab'), $redirect_text );
			else
	...
	function delete()
	{
		...
		if (COMMUNITY_BUILDER_ADSTAB == 1)
			$app->redirect( TRoute::_('index.php?option=com_comprofiler&tab=AdsManagerTab'), JText::_('ADSMANAGER_CONTENT_REMOVED') );
		else
