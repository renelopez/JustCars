			...
			for ($i=0, $n=count( $this->items ); $i < $n; $i++)
				{
					...
					$link = JFilterOutput::ampReplace('index.php?option='.$this->option.'&task=editvehiclecategory&cid[]='.$row->id);
					?>
					<tr valign="top" class="<?php echo "row$k"; ?>">
						...
						<td align="center">
							<?php 
							if($row->status == 1){	?>
								<a href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i;?>','unpublishvehiclecategory')">
									<img src="../components/com_jsautoz/images/tick.png" width="16" height="16" border="0" alt="<?php echo JText::_( 'PUBLISH' ); ?>" /></a>
							<?php }else{	?>
								<a href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i;?>','publishvehiclecategory')">
