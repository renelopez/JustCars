...
Joomla.submitbutton = function(task){
        if (task == ''){
                ...
                if (task == 'savevehiclemodelyear' || task == 'savevehiclemodelyearsave' || task == 'savevehiclemodelyearandnew'){
...
function submitbutton(pressbutton) {
	...
	if(pressbutton == 'savevehiclemodelyear' || pressbutton == 'savevehiclemodelyearsave' || pressbutton == 'savevehiclemodelyearandnew'){
		returnvalue = validate_form(document.adminForm);
	}else returnvalue  = true;
...
<table width="100%" >
	...
	<tr class="row0">
		<td width="20%" valign="top"><label id="titleymsg" for="title"><?php echo JText::_('TITLE'); ?></label>&nbsp;<font color="red">*</font></td>
		  <td width="60%"><input class="inputbox required" type="text" name="title" size="40" maxlength="255" value="<?php if(isset($this->vehiclemodelyear)) echo $this->vehiclemodelyear->title; ?>" />
        </td>
      </tr>
      <tr>
	<tr class="row1">
		<td valign="top"><label id="statusmsg" for="status"><?php echo JText::_('PUBLISHED'); ?></label></td>
		  <td><input type="checkbox" name="status" value="1" <?php if(isset($this->vehiclemodelyear))  {if ($this->vehiclemodelyear->status == '1') echo 'checked';} ?>/>
		  </td>
      </tr>
	<tr><td colspan="2" height="10"></td></tr>
	<tr class="row0">
		...
		<input type="submit" class="button" name="submit_app" onclick="return validate_form(document.adminForm)" value="<?php echo JText::_('SAVE'); ?>" />
			...
			<input type="hidden" name="id" value="<?php if(isset($this->vehiclemodelyear)) echo $this->vehiclemodelyear->id; ?>" />
			<input type="hidden" name="check" value="" />
			<input type="hidden" name="task" value="savevehiclemodelyear" />
