...
function submitbutton(pressbutton) {
	...
	if(pressbutton == 'savevehiclecountry'){
		returnvalue = validate_form(document.adminForm);
	}else returnvalue  = true;
...
<table width="100%" >
	...
	<tr class="row0">
		  ...
		  <input class="inputbox required" type="text" name="name" size="40" maxlength="255" value="<?php if(isset($this->country)) echo $this->country->name; ?>" />
                </td>
      </tr>
      <tr>
	<tr class="row1">
            <td colspan="2" valign="top"><label  id="statusmsg" for="status"><?php echo JText::_('PUBLISHED'); ?></label>
		  <input type="checkbox" name="enabled" value="Y" <?php if(isset($this->country))  {if ($this->country->enabled == 'Y') echo 'checked';} ?>/>
              </td>
      </tr>
	<tr><td colspan="2" height="10"></td></tr>
	<tr class="row0">
		...
		<input type="submit" class="button" name="submit_app" onclick="return validate_form(document.adminForm)" value="<?php echo JText::_('SAVE'); ?>" />
			...
			<input type="hidden" name="task" value="savevehiclecountry" />
