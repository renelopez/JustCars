...
class AdsmanagerTableContents extends JTable
{
	...
	function bindContent($post,$files,$conf,$model,$plugins)
    {
		...
		{ 	
			...
			else if ($field->type == "file")
			{
				if (isset($files[$field->name]) and !$files[$field->name]['error'] ) {
					...
					if (file_exists(JPATH_ROOT."/images/com_adsmanager/files/".$field->name)) {
		...
		foreach($this->data['images'] as $i => $img) {
			if (in_array($img->index,$deleted_images)) {
				JFile::delete(JPATH_ROOT."/images/com_adsmanager/ads/tmp/".$img->image);
				JFile::delete(JPATH_ROOT."/images/com_adsmanager/ads/tmp/".$img->thumbnail);
				@JFile::delete(JPATH_ROOT."/images/com_adsmanager/ads/tmp/".$img->medium);
