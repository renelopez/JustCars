...
class Com_AdsmanagerInstallerScript
{
	...
	protected $_paidsystem_extension = 'com_paidsystem';
	...
	private $paidsystemRemovePlugins = array(
		'paidsystemsubs' => array(
		)
	);
	
	/** @var array Obsolete files and folders to remove */
	private $paidsystemRemoveFiles = array(
		'files'	=> array(
			/*'administrator/components/com_paidsystemsubs/paidsystemsubs.xml',*/
		),
		'folders' => array(
			/*'administrator/components/com_paidsystemsubs/commands',*/
		)
	);
	
	private $paidsystemCliScripts = array();
	...
	function postflight( $type, $parent )
	{
		...
		$paidsystemRemoveFiles = $this->paidsystemRemoveFiles;
		$this->_removeObsoleteFilesAndFolders($paidsystemRemoveFiles);
	...
	private function _removeObsoletePlugins($parent)
	{
		...
		foreach($this->paidsystemRemovePlugins as $folder => $plugins) {
	...
	private function _copyCliFiles($parent)
	{
		...
		if(empty($this->paidsystemCliScripts)) {
			return;
		}
		
		foreach($this->paidsystemCliScripts as $script) {
...
?>
<?php $rows = 0;?>
<h2 style="font-size: 14pt; font-weight: black; padding: 0; margin: 0 0 0.5em;">&nbsp;PaidSystem Uninstallation</h2>
<p>We are sorry that you decided to uninstall PaidSystem. Please let us know why by using the Contact Us form on our site. We appreciate your feedback; it helps us develop better software!</p>
...
<table class="adminlist">
	...
	<tbody>
		<tr class="row0">
			<td class="key" colspan="2"><?php echo 'PaidSystem '.JText::_('Component'); ?></td>
...
<?php
	...
	private function _bugfixDBFunctionReturnedNoError()
	{
		...
		$query->select('id')
			->from('#__assets')
			->where($db->qn('name').' = '.$db->q($this->_paidsystem_extension));
		$db->setQuery($query);
		$ids = $db->loadColumn();
		...
		$query->select('extension_id')
			->from('#__extensions')
			->where($db->qn('element').' = '.$db->q($this->_paidsystem_extension));
		$db->setQuery($query);
		$ids = $db->loadColumn();
		if(!empty($ids)) foreach($ids as $id) {
			$query = $db->getQuery(true);
		...
		$query->select('id')
			...
			->where($db->qn('link').' LIKE '.$db->q('index.php?option='.$this->_paidsystem_extension));
		$db->setQuery($query);
		$ids = $db->loadColumn();
		if(!empty($ids)) foreach($ids as $id) {
			$query = $db->getQuery(true);
	...
	if(!file_exists(JPATH_ROOT. "/images/com_adsmanager/ads/tmp/index.html")){
		JFile::copy(JPATH_ROOT."/components/com_adsmanager/index.html",JPATH_ROOT. "/images/com_adsmanager/ads/tmp/index.html");
	};
	...
	if ($total == 0)
	{
		...
		$queries[] = " INSERT IGNORE INTO `#__adsmanager_fields` (`fieldid`, `name`, `title`, `description`, `type`, `maxlength`, `size`, `required`, `ordering`, `cols`, `rows`, `columnid`, `columnorder`, `pos`, `posorder`, `profile`, `cb_field`, `sort`, `sort_direction`, `published`) VALUES "
...
<?php 
	 ...
	 * 
	 * @param array $paidsystemRemoveFiles 
	 */
	private function _removeObsoleteFilesAndFolders($paidsystemRemoveFiles)
	{
		// Remove files
		jimport('joomla.filesystem.file');
		if(!empty($paidsystemRemoveFiles['files'])) foreach($paidsystemRemoveFiles['files'] as $file) {
			$f = JPATH_ROOT.'/'.$file;
		...
		// Remove folders
		jimport('joomla.filesystem.file');
		if(!empty($paidsystemRemoveFiles['folders'])) foreach($paidsystemRemoveFiles['folders'] as $folder) {
			$f = JPATH_ROOT.'/'.$folder;
	...
	private function _killUpdateSite()
	{
		...
		$query = $db->getQuery(true)
			...
			->where($db->qn('e').'.'.$db->qn('element').' = '.$db->q($this->_paidsystem_extension))
		;
		$db->setQuery($query);
